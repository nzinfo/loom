...
| 维度 | EAV typed（每字段一行） | JSONB 扩展组（每组一行） |
|---|---|---|
...
| 范围查询 | ✓ typed 列可原生索引 | △ 需表达式索引 |
| DB 类型约束 | ✓ 列有精度/长度约束 | △ JSON 弱类型（数值无精度/长度约束） |
| 多租户 | ✓ | ✓ |

**核心权衡**：扩展组用"字段级审计 + 原生索引 + 类型约束 + 字段级并发"换取"行数骤减 + 写入原子性 + 查询简化"。对 ERP 典型行为模式（整体写入、全量读取、偶尔过滤、多字段业务概念需原子更新），扩展组更合适。

行数收益取决于文件划分——组是文件级的，把字段塞进 1 个文件就是 1 行，拆成 100 个文件就退化成和 EAV 一样的行数。分组应按"一起读写的业务概念"来设计（如 profile 组、finance 组）。
