...
- `parsed: Map<identity, AnyFile>` — 节点定义类（type / table / entity），
  identity 唯一，供 `$ref` 查表
...
- field 的单段短名必须能在 using 命名空间（默认含 `base.core.*`）里解析到
  某个 type 节点（scalar/struct/enum 统一，不分流）；解析不到报 `unknown type`
