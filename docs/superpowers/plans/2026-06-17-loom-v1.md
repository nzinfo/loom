...
describe('projector expand', () => {
  ...
  it('records the view name for sidecar_eav tables', async () => {
    const { ir } = await load({ fs: buildBaseSchemaFs(), basePath: '' });
    const phys = expandTables(ir);
    const users = phys.tables.find((t) => t.name === 'users_base');
...
export interface PhysicalColumn {
  readonly name: string;
  /** Logical scalar name from base_types (e.g. "decimal", "string"). */
...
```typescript
/**
 * design → physical expansion. See spec §8.4, §5.5, §6.5.
 *
 ...
 * and records extension-strategy metadata. The result is dialect-neutral;
 * dialects turn it into DDL.
 */
import type { IR } from '../ir/version.js';
import type { Table, ValueType } from '../ir/schemas.js';
import {
  expandValueColumns,
  isSingleFieldValueType,
} from '../ir/field.js';
...
export function expandTables(ir: IR): PhysicalModel {
  ...
  for (const node of ir.nodes.values()) {
    ...
    out.push(expandTable(node.data as Table, node.identity, ir));
  }
  return { tables: out };
}

function expandTable(t: Table, identity: string, ir: IR): PhysicalTable {
  const columns: PhysicalColumn[] = [];
  for (const f of t.fields) {
    if ('include' in f) continue; // already expanded by Pass 2
    if ('ref' in f) {
      const vtNode = ir.nodes.get(f.ref);
      // Pass 2 already flagged dangling refs; skip silently if missing.
      if (!vtNode || vtNode.kind !== 'value_type') continue;
      const vt = vtNode.data as ValueType;
      const cols = expandValueColumns(f.name, vt);
      for (const c of cols) {
        if (isSingleFieldValueType(vt)) {
          const inner = vt.fields[0];
          columns.push(toColumn(c.name, inner as Record<string, unknown>, f.required === true, f.unique === true));
        } else {
          // Map each subfield; <prefix>_<subname> carries the subfield's props.
          const sub = vt.fields.find((sf) => `${f.name}_${sf.name}` === c.name);
          if (sub) {
            columns.push(toColumn(c.name, sub as Record<string, unknown>, (sub as { required?: boolean }).required === true || f.required === true, f.unique === true && cols.length === 1));
          }
        }
      }
      continue;
    }
    columns.push(toColumn(f.name, f as Record<string, unknown>, f.required === true, f.unique === true));
  }
  ...
  return {
    schema: undefined, // filled in by the dialect layer from module_manifest
    name: t.table.name,
    columns,
...
function toColumn(
  ...
  for (const [k, v] of Object.entries(raw)) {
    if (k === 'name' || k === 'base' || k === 'ref' || k === 'required' || k === 'unique' || k === 'default') continue;
    props[k] = v;
  }
  void identity;
  return { name, scalar, props, required, unique };
}
```

> Note: the unused `identity` parameter is `void`-ed. If the implementation prefers, the parameter can be removed and the caller updated. Either is fine; remove it for cleanliness.
...
git commit -m "feat(core): physical model + design→physical expansion (spec §8.4, §5.5, §6.5)"
```
...
```typescript
/**
 * PostgreSQL dialect. See spec §8.4.
 */
import type { PhysicalColumn, PhysicalModel, PhysicalTable } from '../types.js';

export function projectPg(m: PhysicalModel): string {
  return m.tables.map(tableToSql).join('\n\n');
}

function tableToSql(t: PhysicalTable): string {
  const lines: string[] = [];
  lines.push(`CREATE TABLE ${t.name} (`);
  const body: string[] = [];
  for (const c of t.columns) {
    body.push(`  ${c.name} ${pgType(c)}${c.required ? ' NOT NULL' : ''}${c.unique ? ' UNIQUE' : ''}`);
  }
  ...
  for (const idx of t.indexes) {
    lines.push(
      `CREATE ${idx.unique ? 'UNIQUE ' : ''}INDEX ${idx.name} ON ${t.name} (${idx.columns.join(', ')});`,
    );
  }
  if (t.strategy === 'sidecar_eav' && t.viewName) {
    lines.push(`CREATE VIEW ${t.viewName} AS SELECT * FROM ${t.name};`);
  }
  return lines.join('\n');
}

function pgType(c: PhysicalColumn): string {
  switch (c.scalar) {
    case 'boolean':
      return 'BOOLEAN';
    case 'integer':
...
```typescript
/**
 * MySQL dialect. See spec §8.4.
 */
import type { PhysicalColumn, PhysicalModel, PhysicalTable } from '../types.js';

export function projectMysql(m: PhysicalModel): string {
  return m.tables.map(tableToSql).join('\n\n');
}
...
function tableToSql(t: PhysicalTable): string {
  ...
  const body: string[] = [];
  for (const c of t.columns) {
    body.push(`  ${c.name} ${mysqlType(c)}${c.required ? ' NOT NULL' : ''}${c.unique ? ' UNIQUE' : ''}`);
  }
  if (t.primaryKey.length > 0) {
    body.push(`  PRIMARY KEY (${t.primaryKey.join(', ')})`);
  }
...
function mysqlType(c: PhysicalColumn): string {
  switch (c.scalar) {
    case 'boolean':
      return 'BOOLEAN';
    case 'integer':
...
```typescript
/**
 * SQLite dialect. See spec §8.4.
 *
 * SQLite is dynamically typed; we still emit nominal types for tooling.
 * enum would normally use CHECK here; v1 emits TEXT and lets validate()
 * enforce the value set semantically.
 */
import type { PhysicalColumn, PhysicalModel, PhysicalTable } from '../types.js';

export function projectSqlite(m: PhysicalModel): string {
  return m.tables.map(tableToSql).join('\n\n');
}
...
export function projectSql(m: PhysicalModel, dialect: Dialect): string {
  switch (dialect) {
    case 'pg':
      return projectPg(m);
    case 'mysql':
      return projectMysql(m);
    case 'sqlite':
      return projectSqlite(m);
  }
}
```
...
git commit -m "feat(core): PG/MySQL/SQLite dialects + sql projector (spec §8.3, §8.4)"
```
...
import { load, expandTables, projectSql, type Dialect, type FileSystem } from '@loom/core';
...
export async function projectCommand(opts: ProjectOptions): Promise<number> {
  ...
  const model = expandTables(result.ir);
  const sql = projectSql(model, dialect);
...
describe('golden: base_schema.pg.sql', () => {
  it('matches the committed golden file', async () => {
    const { ir } = await load({ fs: buildBaseSchemaFs(), basePath: '' });
    const sql = projectSql(expandTables(ir), 'pg');
...
- [ ] **Step 3: Write a small node script to dump the current SQL, save as golden**

Run from repo root:

```bash
node --input-type=module -e "
import { load, expandTables, projectSql } from './packages/core/dist/index.js';
import { buildBaseSchemaFs } from './packages/core/tests/fixtures/base_schema.js';
const { ir } = await load({ fs: buildBaseSchemaFs(), basePath: '' });
process.stdout.write(projectSql(expandTables(ir), 'pg'));
" > packages/core/tests/golden/base_schema.pg.sql
```

If the dynamic import of the test fixture fails under node directly, instead run via vitest by temporarily commenting the assertion and `console.log(sql)`, then capture stdout into the golden file. The simpler path: copy the SQL output from the previous failing run if it printed a diff.
...
**Known limitations (call out to user before execution):**
1. **Enum projection** (§11) is not implemented in this plan — the SQL projector treats `enum` base as `TEXT`-default. The base_types fixture does not include `enum`, so tests pass. If enum support is required for v1, add a task between 11 and 12 to handle `values: [...]` in `toColumn` + per-dialect rendering (PG `CREATE TYPE`, MySQL `ENUM(...)`, SQLite `CHECK`).
2. **Sidecar EAV view** is a stub `SELECT * FROM base`. The real pivot view (spec §7.4) is follow-up work; `extension_fields` is validated but its content does not yet shape the view.
3. **Module manifest `physical_schema`** is parsed but the projector does not prefix table names with it. Spec §6.2 expects `base_core.users_base`. Add to Task 11 if full physical-schema prefixing is needed in v1.
4. **Line/column in diagnostics** is hardcoded `1:1`. Real positions require a YAML parser that emits CST nodes; this is a deliberate simplification to keep v1 shippable.
